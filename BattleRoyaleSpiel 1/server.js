...
io.on('connection', (socket) => {
    ...
    socket.on('shootPlayer', (data) => {
        ...
        if (target) {
            if (target.shield > 0) {
                ...
                if (target.shield < 0) { target.hp += target.shield; target.shield = 0; }
            } else {
    ...
    socket.on('playerShoot', (data) => {
        // Broadcast bullet trail to everyone else
    ...
    socket.on('requestRespawn', () => {
        ...
        if(p) {
            ...
            p.x = (Math.random()-0.5)*100; p.z = (Math.random()-0.5)*100; p.y = 10;
